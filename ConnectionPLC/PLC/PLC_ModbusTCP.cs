...
{
    public class PLC_ModbusTCP: PLC_Base
    {
        ...
        public PLC_ModbusTCP(string ip, int port = 502, int sizeDataRead = 24000, ushort startAddress = 0x3000, ushort maxSizeRegReadCmd = 100, bool checkCRC = false) : base(ip, port, sizeDataRead)
        {
            IP = ip;
            Port = port;
                ...
                if (client?.Connected ?? false && master != null)
                {
                    ...
                    for (int i = 0; i < numberReadRegisters; i += maxSizeRegReadCmd)
                    {
                        ...
                        lock (locker)
                        {
                            try
                            {
                                inputs = master?.ReadHoldingRegisters(1, (ushort)(startAddress + i), maxSizeRegReadCmd);
                            }
                            catch (Exception ex)
                            {
                    ...
                    if (k == SizeDataRead)
                    {
                        ...
                        if (checkCRC)
                        {
                            ...
                            for (int i = 0; i < 9; i++)
                            {
                                crcReceived[i] = BitConverter.ToUInt16(memory.Slice(1000 + (i * 2), 2).Span.ToArray(), 0);
                            }
                            int t = 0;
                        ...
                        if (correctCRC)
                        {
                            // context.Post(ReadComleted, null); //Для WPF
        ...
        public override bool Write(int adr, object value)
        {
            ...
            switch (value)
            {
                ...
                case ushort ushortVal:
                    ...
                    break;
                case uint uintVal:
                    result = WriteUintFloatDt(uintVal);
                    break;
                case float floatVal:
                    result = WriteUintFloatDt(floatVal);
                    break;
                case DateTime DateTimeVal:
                    result = WriteUintFloatDt(value);
                    break;
                case TimeSpan TimeSpanVal:
                    result = WriteUintFloatDt(value);
                    break;
                default:
            ...
            bool WriteBoolByte(int addr, object val)
            {
                bool writeOk = false;
                int nByte = 0;
                if (addr % 2 != 0)
                ...
                (ushort reg, bool ok) = GetRegister(addr);
                if (ok)
                {
                    var bytes = BitConverter.GetBytes(reg);
                    //bytes[nByte] = val switch
                    //{
                    //    bool boolVal => (byte)(boolVal ? 1 : 0),
                    //    byte byteVal => byteVal,
                    //    _ => throw new ArgumentException($"{nameof(WriteBoolByte)}:{nameof(val)}"),
                    //};
                    var writeRegister = BitConverter.ToUInt16(bytes, 0);
                    writeOk = WriteSingleRegister(addr, writeRegister);
                }
                return writeOk;
            }

            bool WriteUintFloatDt(object vValue)
            {

                uint uintVal = new uint();
                float floatVal = new float();
                DateTime DateTimeVal = new DateTime();
                TimeSpan TimeSpanVal = new TimeSpan();

                var @switch1 = new Dictionary<object, byte[]> {
    { typeof(uint), BitConverter.GetBytes(uintVal) },
    { typeof(uint), BitConverter.GetBytes(floatVal) },
};
                @switch1.TryGetValue(vValue, out var arr);
        ...
        public bool WriteBit(WORD tag, int numBit, bool val)
        {
        ...
        private bool WriteSingleRegister(int mbAddress, ushort val, ushort[] values = null)
        {
            if (master == null) return false;
            bool writeOk = false;
            ...
            lock (locker)
            {
                try
                {
                    ...
                    {
                        master?.WriteSingleRegister(0, registerAddress, val);
                    }
                    writeOk = true;
                }
                catch (Exception ex)
