...
namespace ConnectionPLC
{
    ...
    {
        private  Action plcGetValue;
        protected T value;  // значение тега
        protected PLC_Base plc;  // класс связи с ПЛК
        ...
        public Tag(int adr, PLC_Base plc)
        {
        ...
        protected virtual void PlcReadComleted()
        {
            var switchTypeDict = new Dictionary<Type, Action> {
    { typeof(bool), () => { bool v = plc.GetBOOL(Adr); Value = Unsafe.As<bool, T>(ref v); } },
    { typeof(byte), () => { byte v = plc.GetBYTE(Adr); Value = Unsafe.As<byte, T>(ref v); } },
    { typeof(ushort), () => { ushort v = plc.GetWORD(Adr); Value = Unsafe.As<ushort, T>(ref v); } },
    { typeof(uint),() => { uint v = plc.GetDWORD(Adr); Value = Unsafe.As<uint, T>(ref v); } },
    { typeof(ulong),() => { ulong v = plc.GetLWORD(Adr); Value = Unsafe.As<ulong, T>(ref v); } },
    { typeof(sbyte),() => { sbyte v = plc.GetSBYTE(Adr); Value = Unsafe.As<sbyte, T>(ref v); } },
    { typeof(short) , () => { short v = plc.GetINT(Adr); Value = Unsafe.As<short, T>(ref v); } },
    { typeof(int), () => { int v = plc.GetDINT(Adr); Value = Unsafe.As<int, T>(ref v); } },
    { typeof(long), () => { long v = plc.GetLINT(Adr); Value = Unsafe.As<long, T>(ref v); } },
    { typeof(float), () => { float v = plc.GetREAL(Adr); Value = Unsafe.As<float, T>(ref v); } },
    { typeof(double), () => { double v = plc.GetLREAL(Adr); Value = Unsafe.As<double, T>(ref v); } },
    { typeof(DateTime), () => { DateTime v = plc.GetDT(Adr); Value = Unsafe.As<DateTime, T>(ref v); } }

};
            //int v = plc.GetDINT(Adr); 
            //Value = Unsafe.As<int, T>(ref v);
            //Action plcGetValue = new;
            switchTypeDict.TryGetValue(typeof(T), out plcGetValue);
            plcGetValue();
        }
